        ...
        function startAttendanceListener() {
            ...
            dbRef.off(); // remove previous listeners if any
            ...
            dbRef.on("value", (snapshot) => {
                ...
                snapshot.forEach((studentSnap) => {
                    const sessions = studentSnap.child("sessions");
                    if (sessions.exists()) {
                        sessions.forEach((sessionSnap) => {
                            const student = sessionSnap.val();
                            allStudents.push({ snap: sessionSnap, data: student });
                        });
                        loggedInCount++;
                    }
                });
